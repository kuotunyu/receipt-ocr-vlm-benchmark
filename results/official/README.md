...
每份 artifact 都包含資料集識別、六個正式配置、README 使用的 headline metrics、完整聚合指標，以及來源 `summary.json` 的 SHA-256。逐張預測、OCR 文字、圖片與執行期錯誤訊息不在公開 artifact 中。
...
這兩個原始實驗資料夾刻意不進版控；保留正式評估工作區的人可以用 `--check` 重建查核。Exporter 採欄位白名單，遇到未知欄位、配置缺漏、樣本數不符或非有限數字會直接停止，輸出也不包含時間戳，因此相同來源會得到 byte-for-byte 相同的檔案。
